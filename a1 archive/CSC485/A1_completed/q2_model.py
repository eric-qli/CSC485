...
class GraphDepModel(nn.Module):
    ...
    def create_arc_layers(self) -> None:
        ...
        * Initialize self.arc_W and self.arc_B according to a uniform
        ...
        Returns:
            None
        """
        # Create two transformation layers for heads and dependents using nn.Sequential
        # Input size: self.pt_width, hidden size: cfg.n_arcs, output size: cfg.n_arcs
        ...
        # arc_W is a 2D weight matrix for biaffine transformation
        # Dimensions: (cfg.n_arcs, cfg.n_arcs) based on expected usage for biaffine scoring
        limit = sqrt(3 / self.cfg.n_arcs)
        self.arc_W = nn.Parameter(torch.empty(self.cfg.n_arcs, self.cfg.n_arcs))
        nn.init.uniform_(self.arc_W, -limit, limit)

        # arc_B is the bias vector, size: (cfg.n_arcs,)
        self.arc_B = nn.Parameter(torch.empty(self.cfg.n_arcs))
        nn.init.uniform_(self.arc_B, -limit, limit)
    ...
    def score_arcs(self, arc_head: Tensor, arc_dep: Tensor) -> Tensor:
        ...
        """
        # *** ENTER YOUR CODE BELOW *** #
        bilinear_term = torch.einsum('bxi,ij,byj->byx', arc_head, self.arc_W, arc_dep)
        bias_term = torch.einsum('bxi,i->bx', arc_head, self.arc_B)
        arc_tensor = bilinear_term + bias_term.unsqueeze(1)
    ...
    def create_label_layers(self) -> None:
        ...
        * Initialize self.label_B to zeros.
        ...
        """
        # *** ENTER YOUR CODE BELOW *** #
        # Create MLP layers for the head and dependent transformations
        ...
        # Calculate the limit for uniform initialization using torch.sqrt
        limit = sqrt(torch.tensor(3.0 / self.cfg.n_labels))

        self.label_W = nn.Parameter(torch.empty(self.cfg.n_labels, self.cfg.n_labels, self._n_deprels))
        nn.init.uniform_(self.label_W, -limit, limit)

        self.label_h_W = nn.Parameter(torch.empty(self.cfg.n_labels, self._n_deprels))
        nn.init.uniform_(self.label_h_W, -limit, limit)

        self.label_d_W = nn.Parameter(torch.empty(self.cfg.n_labels, self._n_deprels))
        nn.init.uniform_(self.label_d_W, -limit, limit)
    ...
    def score_labels(self, label_head: Tensor, label_dep: Tensor) -> Tensor:
        ...
        """
        # *** ENTER YOUR CODE BELOW *** #
        biaffine_term = torch.einsum('bxi,ijk,byj->byxk', label_head, self.label_W, label_dep)
        head_only_term = torch.einsum('bxi,ik->bxk', label_head, self.label_h_W)
        dep_only_term = torch.einsum('byi,ik->byk', label_dep, self.label_d_W)

        # Broadcast the head-only term over the y dimension, and the dependent-only term over the x dimension
        combined_scores = biaffine_term + head_only_term.unsqueeze(1) + dep_only_term.unsqueeze(2)
        label_scores = combined_scores + self.label_B
        return label_scores
    ...
    def mask_possible(self, shape: T.Sequence[int]):
        ...
        batch_size, y, x, n_deprels = shape

        for i in range(y):
            mask[:, i, i, :] = False  # No self-loop for vertex i

        mask[:, 0, :, 0] = False  # No dependency to ROOT (vertex 0) with root relation
