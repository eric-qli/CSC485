...
class ParserModel(nn.Module):
    ...
    def create_net_layers(self) -> None:
        ...
        """
        # *** ENTER YOUR CODE BELOW *** #
        # Calculate the size of each input component
        n_word_features = self.config.n_word_features   
        n_tag_features = self.config.n_tag_features
        n_deprel_features = self.config.n_deprel_features

        # Calculate the total input size
        input_size = (n_word_features + n_tag_features + n_deprel_features) * self.config.embed_size
    ...
    def reshape_embedded(self, input_batch: torch.Tensor) -> torch.Tensor:
        ...
        """
        # *** ENTER YOUR CODE BELOW *** #
        reshaped_batch = einops.rearrange(input_batch, 'b n e -> b (n e)')
    ...
    def concat_embeddings(self, word_id_batch: torch.Tensor,
        ...
        # *** ENTER YOUR CODE BELOW *** #
        # Retrieve embeddings
        word_embeddings = self.word_embed(word_id_batch)   # Shape: (B, n_word_features, embed_size)
        tag_embeddings = self.tag_embed(tag_id_batch)      # Shape: (B, n_tag_features, embed_size)
        deprel_embeddings = self.deprel_embed(deprel_id_batch)  # Shape: (B, n_deprel_features, embed_size)

        # Concatenate embeddings, second dimension
        concatenated_embeddings = torch.cat([word_embeddings, tag_embeddings, deprel_embeddings], dim=1)
        
        # flatten the embeddings
        reshaped_embeddings = self.reshape_embedded(concatenated_embeddings)
    ...
    def forward(self,
        ...
        h_relu = torch.relu(h)
        h_drop = torch.dropout(h_relu, self.config.dropout, self.training)
